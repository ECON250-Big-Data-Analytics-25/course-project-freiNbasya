...
when lower(platform) in ('linkedin', 'ld') then 'Linkedin'
when lower(platform) in ('snap') then 'Snap'
else platform
...
case
when impressions like '%M' or impressions like '%m' then
safe_cast( regexp_replace(impressions,r"\D","") as int64) * 1000000
when impressions like '%K' or impressions like '%k' then
safe_cast( regexp_replace(impressions,r"\D","") as int64) * 1000
else safe_cast( regexp_replace(impressions,r"\D","") as int64) 
end impressions_fix,

case
when regexp_contains(date, r'^\d{2}/\d{2}/\d{2}$') then
parse_date('%x', date)

when regexp_contains(date, r'^\d{2}/\d{2}/\d{4}$') then
parse_date('%m/%d/%Y', date)

when regexp_contains(date, r'^\d{4}-\d{2}-\d{2}$') then
...
* from {{source('ikondenko', 'sheet_external')}}
